...
void player_reconnect(Players **array_players, int socketID, games **all_games){
    if(get_player_by_socket((*array_players), socketID) == NULL)
        return;

    int client_state = get_player_by_socket((*array_players), socketID)->state;
    Player *this_client = get_player_by_socket((*array_players), socketID);
    ...
    if(client_state == 0) {
        ...
        if(strcmp(this_game->name_1, get_player_by_socket((*array_players), socketID)->name) == 0){
            char message[512];
